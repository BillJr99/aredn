...
docker run --name aredn-builder arednmesh/builder bash -lc '
  ...
  # branches, e.g. also run:  git pull --no-rebase --no-edit -X ours origin mcar/tiny-xm
  # (mcar/tiny-xm additionally enables the experimental ath79/tiny XM target).
  ...
  git pull --no-rebase --no-edit -X ours origin mcar/dhcp &&
